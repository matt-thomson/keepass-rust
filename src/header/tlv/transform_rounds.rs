...
pub fn read_transform_rounds(reader: &mut Read, length: u16) -> Result<Tlv, Error> {
...
mod test {
    ...
    #[test]
    fn should_read_transform_rounds() {
        let mut bytes = vec![];
        bytes.write_u64::<LittleEndian>(1234).unwrap();

        let result = read_transform_rounds(&mut &bytes[..], 8);
    ...
    fn should_return_error_if_wrong_length() {
        ...
        let result = read_transform_rounds(&mut &bytes[..], 7);
